...
export default function PaymentPage({ planId }: Props) {
  ...
  }, [promoApplied, promoRate, subtotalAmount]);
  ...
  // Nội dung chuyển khoản (mock)
  const transferNote = useMemo(() => {
    const suffix = Math.floor(Math.random() * 10000);
    return `DOCUFLOW ${String(effectivePlanId).toUpperCase()} ${suffix}`;
  }, [effectivePlanId]);
  ...
  const handleCopy = async () => {
    const textToCopy = bankInfo.accountNumber;
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-blue-100 relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                ...
                <div className="flex-shrink-0 bg-blue-50 p-4 rounded-xl border-2 border-dashed border-blue-200 text-center">
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=DocuflowAI-${selectedPlan.id}-${totalAmount}`}
                    alt="Payment QR Code"
                ...
                <div className="flex-1 w-full space-y-4">
                  <div>
                    ...
                    <p className="font-medium text-lg text-blue-900">{bankInfo.bankName}</p>
                  </div>
